
/*  1. Vypiste nazvy vsetkych knih, ktore su prave teraz pozicane (t.j. neexistuje zaznam o ich vrateni). */

/*  2. Vypiste meno, datum registracie, den v tyzdni (v den registracie) pre vsetkych citatelov, ktori sa registrovali pocas posledneho roka.
Na vypis dna v tyzdni v ludsky citatelnom formate pouzite CASE statement. */

/*  3. Vypiste vsetky trojice [C, K, DatumPozicania], kde K je nazov knihy, ktoru si citatel s menom C pozical, ale este ju nevratil. */

/*  4. Najdite mena citatelov, ktori vratili do 30 dni kazdu z knih, ktore si pozicali. (Vratane citatelov, ktori si nikdy nepozicali nic.) */

/*  5. Vypiste mena citatelov, ktori si pozicali aspon tri rozne knihy, a pritom vsetky ich pozicane knihy boli od toho isteho autora. */

/*  6. Ku kazdemu citatelovi vypiste nazov prvej knihy, ktoru si pozical, alebo null, ak si nikdy nepozical nic. */

/*  7. Najdite citatelov, ktori si nepozicali nic pocas prveho roka od registracie. */

/*  8. (1 point) Pre kazdeho citatela vypiste dobu jeho najdlhsej ukoncenej vypozicky (pre knihy, ktore uz vratil).
Pre citatelov, ktori si nic nepozicali, vratte NULL. */

/*  Extra: premyslite si, ako upravit 8 tak, aby sa brali do uvahy aj neukoncene vypozicky. */

/* Data modification */

/*  9. Zmente meno prvemu registrovanemu citatelovi pomocou UPDATE. */

/* 10. Zmazte knihy, ktore napisal 'Adolf'. */

/* 11. Zmazte knihy, ktore si uz pozical kazdy registrovany citatel. */

/* 12. (1 point) Pridajte k menu kazdeho citatela, ktory mal v kazdom momente pozicanu nanajvys jednu knihu, retazec " Read more!".
Navod: https://www.postgresql.org/docs/9.1/functions-string.html */
